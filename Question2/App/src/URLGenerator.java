...
public class URLGenerator
{
    ...
    private boolean validateGoogleURL(String url)
    {
        ...
        //4) It can only contain letters (a-z, A-Z), digits (0-9), hyphens (-), forward slashes (/), and periods (.).

        String lowercaseUrl = url.toLowerCase();

        boolean containsGoogle = lowercaseUrl.contains("google");
        boolean validHostname = lowercaseUrl.matches(".*\\bgoogle\\b.*\\.(com|ie).*");
        boolean validLength = url.length() >= 6;
        boolean validCharacters = url.matches("[a-zA-Z0-9/\\-\\.]+");

        return containsGoogle && validHostname && validLength && validCharacters;

        //I cannot make this method work, same as with the previous one in Question 1 regarding the LLD and etc!.
    }
